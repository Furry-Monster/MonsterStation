...
不严格地讲，渲染方程指出空间点在方向上出射的能量等于它自身在该方向发射的能量加上来自半球面上所有方向入射并经过反射后反射到上的能量之和。
...
Cook-Torrance模型可以拆分为三项。首先是 **法向分布函数(normal distribution function)** ，它表示材质不同方向上镜面法向的分布情况。法向分布函数包含一个参数来表示表面的粗糙度，越大表示表面越粗糙反射行为越接近漫反射，越小表示表面越光滑反射行为越接近理想镜面反射。
