...
这里我修改了很多次标题，从图形管线，改到渲染管线、GPU管线，最后确定下来使用GPU渲染管线，主要是因为这些概念非常容易混淆，尽管有些咬文嚼字，但是想构造正确的知识网络，这些区别是必须要弄清楚的。

![img](diff.png)

这张图，我们常说是GPU渲染管线，指代GPU渲染图形的工作过程及其环境，前缀GPU则是用于区分“引擎渲染管线”，同时也说明这是GPU用于渲染的管线，而不是执行的逻辑。
...
注意，请务必区分渲染资源和数据。我们认为.fbx格式的模型是数据文件，而不是渲染资源，只有当它的纹理被分配到通道，网格体被绑定到缓冲下，我们才认为这些通道和缓冲区是**渲染资源。**
