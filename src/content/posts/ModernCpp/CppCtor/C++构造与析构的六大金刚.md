...
系统隐式定义的默认构造为 `T() = default` 。 它从行为上与用户亲自定义一个空的默认构造函数： `T() {}` 没有任何差别: 调用父类 和所有非静态成员的默认构造。所有的基本类型，指针，枚举等，其默认构造什么也不做，其值保持在其被分配出来时，内存中的样子。
...
**copy家族** 和 **move家族** 的这种互斥性，是因为它们从根本上属于同一范畴的问题 。 一旦程序员打算对于这一范畴的问题做出自己的决定，那么编译器任何自作主张的行为都不能保证是安全的。 因而，**move/copy家族** ，编译器奉行的是 nothing or all 的策略：要么完全由编译器自动生成，要么完全由用户自己决定。
...
在 C++ 中，如果你的类不手动管理任何资源（例如原始指针或文件句柄），而是依赖于标准容器或智能指针，那么**Rule of 0** 就适用。在这种情况下，你不需要编写任何特殊函数，例如析构函数或复制构造函数；编译器会自动为你生成它们，从而使你的代码更简洁、更安全。
...
public:
    ...
    Person(const string& n, int a)
        : name(make_unique<string>(n)), age(a) {}

    // Display function using std::format
    void display() const {
...
int main() {
    ...
    // Move the object (unique_ptr can't be copied)
...
**Rule of 3 — You Manage Resources**

当你的类需要手动管理资源时（例如使用 `new` 和 `delete`），**Rule of 3** 就显得尤为重要。在这种情况下，你必须定义3个特殊的成员函数：析构函数、复制构造函数和复制赋值运算符。这些函数确保你的类能够正确地处理复制和清理操作，从而防止内存泄漏、浅拷贝或重复删除等问题。
...
public:
    ...
    MyArray& operator=(const MyArray& other) { /* deep copy */ return *this; }
};
```

**Rule of 5 — Want Performance? Add Move Support**

**Rule of 5** 扩展了**Rule of 3**，增加了对移动语义的支持，这对于性能优化非常有用。如果你的类管理资源，并且你希望实现高效的所有权转移（而不是复制），那么你还应该定义一个移动构造函数和一个移动赋值运算符。这两个附加函数使得快速移动资源成为可能，尤其是在处理大型或临时对象时。这些规则共同帮助你编写更安全、更高效、更易于维护的 C++ 代码。

```cpp
MyArray(MyArray&& other) noexcept { /* move constructor */ }
MyArray& operator=(MyArray&& other) noexcept { /* move assignment */ return *this; }
```
